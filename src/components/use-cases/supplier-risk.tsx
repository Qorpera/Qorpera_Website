      ...
      <div style={{ width: "100%", maxWidth: 1080, padding: "0 40px", minHeight: 400, position: "relative", display: "flex", alignItems: "center", justifyContent: "center" }}>
        ...
        {show("title") && (
          <div style={{ textAlign: "center" }}>
            ...
            <Fade show delay={500} duration={800}><div style={{ fontSize: 60, fontWeight: 700, color: "#f1f5f9", fontFamily: "'Space Grotesk'", marginBottom: 16, lineHeight: 1.2 }}>The Supplier About to Fail</div></Fade>
            <Fade show delay={1000} duration={700}><div style={{ fontSize: 24, color: "#94a3b8", maxWidth: 620, margin: "0 auto", lineHeight: 1.7 }}>Payment terms changed. Delivery windows stretched. Quality complaints up 40%. Your key supplier is in trouble — the signals are in three departments.</div></Fade>
            <Fade show delay={2200} duration={600}><div style={{ fontSize: 19, color: "#64748b", marginTop: 24, fontStyle: "italic" }}>By the time a supplier formally notifies you,<br/>it&apos;s already too late.</div></Fade>
          </div>
        )}
        ...
        {show("the-signals") && (
          <div style={{ display: "flex", alignItems: "center", gap: 50, width: "100%", padding: "0 60px" }}>
            <div style={{ flex: 1 }}>
              <Fade show delay={200} duration={600}><div style={{ fontSize: 15, fontWeight: 600, letterSpacing: 3, color: "#06b6d4", textTransform: "uppercase", marginBottom: 16, fontFamily: "'JetBrains Mono'" }}>Warning Signs</div></Fade>
              {[
                { dept: "Finance", signal: "Supplier requested 60-day payment terms (was 30 days).", color: "#f59e0b" },
                { dept: "Receiving", signal: "3 of last 5 deliveries arrived late. Average slip: 4 days.", color: "#ef4444" },
                { dept: "QA", signal: "Defect rate increased from 2% to 5.6% over last quarter.", color: "#a855f7" },
                { dept: "Procurement email", signal: "\"Heard they laid off 30% of their production team last month.\"", color: "#3b82f6" },
              ].map((row, i) => (
                <Fade key={i} show delay={400 + i * 550} duration={500}>
                  <div style={{ display: "flex", gap: 12, padding: "12px 16px", background: `${row.color}06`, border: `1px solid ${row.color}18`, borderRadius: 10, marginBottom: 8 }}>
                    <div>
                      <div style={{ fontSize: 15, color: row.color, fontFamily: "'JetBrains Mono'", marginBottom: 4, fontWeight: 600 }}>{row.dept}</div>
                      <div style={{ fontSize: 18, color: "#e2e8f0", fontWeight: 500, lineHeight: 1.5 }}>{row.signal}</div>
                    </div>
                  </div>
                </Fade>
              ))}
            ...
            <div style={{ flex: 1 }}>
              <Fade show delay={600} duration={700}><div style={{ fontSize: 39, fontWeight: 700, color: "#f1f5f9", fontFamily: "'Space Grotesk'", marginBottom: 12, lineHeight: 1.3 }}>Four departments.<br/>Four red flags.<br/><span style={{ color: "#ef4444" }}>Zero connection.</span></div></Fade>
              <Fade show delay={1400} duration={600}><div style={{ fontSize: 21, color: "#94a3b8", lineHeight: 1.7, maxWidth: 380 }}>Finance sees a payment request. Receiving sees late trucks. QA sees bad parts. Procurement hears a rumour. Nobody connects the dots.</div></Fade>
            </div>
          </div>
        )}
        ...
        {show("the-risk") && (
          <div style={{ display: "flex", alignItems: "center", gap: 50, width: "100%", padding: "0 60px" }}>
            <div style={{ flex: 1 }}>
              <Fade show delay={200} duration={600}><div style={{ fontSize: 15, fontWeight: 600, letterSpacing: 3, color: "#ef4444", textTransform: "uppercase", marginBottom: 16, fontFamily: "'JetBrains Mono'" }}>Dependency Analysis</div></Fade>
              <Fade show delay={500} duration={500}>
                <div style={{ padding: "24px 28px", background: "#12122a", border: "1px solid #1e1e3a", borderRadius: 14 }}>
                  {[
                    { label: "Components supplied", value: "3 critical parts", color: "#e2e8f0" },
                    { label: "Alternate suppliers", value: "None qualified", color: "#ef4444" },
                    { label: "Qualification lead time", value: "4 months", color: "#f59e0b" },
                    { label: "Current inventory cover", value: "6 weeks", color: "#f59e0b" },
                    { label: "Risk if supplier fails", value: "Production stops", color: "#ef4444" },
                  ].map((row, i) => (
                    <Fade key={i} show delay={700 + i * 400} duration={400}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", borderBottom: i < 4 ? "1px solid #1e1e3a" : "none" }}>
                        ...
                        <span style={{ fontSize: 18, color: row.color, fontWeight: 600 }}>{row.value}</span>
                      </div>
                    </Fade>
                  ))}
                </div>
            ...
            <div style={{ flex: 1 }}>
              <Fade show delay={600} duration={700}><div style={{ fontSize: 39, fontWeight: 700, color: "#f1f5f9", fontFamily: "'Space Grotesk'", marginBottom: 12, lineHeight: 1.3 }}>If they go down,<br/><span style={{ color: "#ef4444" }}>you go down 4 months later.</span></div></Fade>
              <Fade show delay={1400} duration={600}><div style={{ fontSize: 21, color: "#94a3b8", lineHeight: 1.7, maxWidth: 380 }}>Three critical components, no backup supplier, and a 4-month qualification process. Your 6 weeks of inventory is all that stands between you and a production halt.</div></Fade>
            </div>
          </div>
        )}
        ...
        {show("detection") && (
          <div style={{ textAlign: "center", width: "100%", padding: "0 50px" }}>
            <Fade show delay={200} duration={600}><div style={{ fontSize: 18, fontWeight: 600, letterSpacing: 3, color: "#06b6d4", textTransform: "uppercase", marginBottom: 16, fontFamily: "'JetBrains Mono'" }}>Qorpera sees the pattern</div></Fade>
            <Fade show delay={600} duration={800}>
              <div style={{ maxWidth: 700, margin: "0 auto 24px", padding: "20px 28px", background: "#06b6d408", border: "1px solid #06b6d420", borderRadius: 14, textAlign: "left" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
                  ...
                  <span style={{ fontSize: 20, fontWeight: 700, color: "#ef4444" }}>SUPPLIER RISK: Supplier ABC showing 4 concurrent risk indicators</span>
                </div>
                <div style={{ fontSize: 19, color: "#cbd5e1", lineHeight: 1.8 }}>
                  Late deliveries (60%), quality decline (5.6% defect rate), payment term extension (30&rarr;60 days), workforce reduction (30%). Dependency level: CRITICAL — 3 components, no backup supplier.
                </div>
              </div>
            </Fade>
            <Fade show delay={2200} duration={700}>
              <div style={{ display: "flex", justifyContent: "center", gap: 24, flexWrap: "wrap" }}>
                {[
                  { value: "60%", label: "Deliveries late", color: "#ef4444" },
                  { value: "5.6%", label: "Defect rate", color: "#a855f7" },
                  { value: "30→60d", label: "Payment terms", color: "#f59e0b" },
                  { value: "-30%", label: "Workforce cut", color: "#ef4444" },
                ].map((stat, i) => (
        ...
        {show("action") && (
          <div style={{ textAlign: "center", width: "100%", padding: "0 50px" }}>
            ...
            <Fade show delay={500} duration={800}><div style={{ fontSize: 45, fontWeight: 700, color: "#f1f5f9", fontFamily: "'Space Grotesk'", marginBottom: 28, lineHeight: 1.3 }}>Act now, or scramble <span style={{ color: "#ef4444" }}>in 6 weeks.</span></div></Fade>
            <Fade show delay={1200} duration={500}>
              <div style={{ maxWidth: 640, margin: "0 auto" }}>
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  {[
                    { n: "1", text: "Initiate backup supplier qualification immediately — 4-month lead time starts now", color: "#ef4444" },
                    { n: "2", text: "Build safety stock for critical components — extend buffer to 8 weeks", color: "#f59e0b" },
                    { n: "3", text: "Schedule supplier review meeting this week — address delivery and quality trends", color: "#06b6d4" },
                    { n: "4", text: "Alert production planning to adjust forecasts for potential supply disruption", color: "#3b82f6" },
                  ].map((a, i) => (
        ...
        {show("close") && (
          <div style={{ textAlign: "center" }}>
            <Fade show delay={200} duration={800}><div style={{ fontSize: 53, fontWeight: 700, color: "#f1f5f9", fontFamily: "'Space Grotesk'", marginBottom: 16, lineHeight: 1.3 }}>Procurement tracks suppliers.<br/><span style={{ color: "#06b6d4" }}>Qorpera tracks supplier health.</span></div></Fade>
            <Fade show delay={1000} duration={700}><div style={{ fontSize: 24, color: "#94a3b8", maxWidth: 560, margin: "0 auto 28px", lineHeight: 1.7 }}>Payment terms, delivery patterns, quality trends, workforce signals — scattered across four departments. Activity intelligence connects them into one risk picture before the supplier formally notifies you.</div></Fade>
