...
const SCENES: Scene[] = [
  ...
  { id: "the-discovery", duration: 9625 },
      ...
      <div style={{ width: "100%", maxWidth: 1080, padding: "0 40px", minHeight: 400, position: "relative", display: "flex", alignItems: "center", justifyContent: "center" }}>
        ...
        {show("title") && (
          <div style={{ textAlign: "center" }}>
            ...
            <Fade show delay={500} duration={800}><div style={{ fontSize: 60, fontWeight: 700, color: "#f1f5f9", fontFamily: "'Space Grotesk'", marginBottom: 16, lineHeight: 1.2 }}>The Tool Nobody<br/>Approved</div></Fade>
            <Fade show delay={1000} duration={700}><div style={{ fontSize: 24, color: "#94a3b8", maxWidth: 560, margin: "0 auto", lineHeight: 1.7 }}>Sales team signed up for a file-sharing service. Customer data is now in an unencrypted third-party system. Security doesn&apos;t know. 400 files uploaded.</div></Fade>
            <Fade show delay={2200} duration={600}><div style={{ fontSize: 19, color: "#64748b", marginTop: 24, fontStyle: "italic" }}>Shadow IT doesn&apos;t start with bad intentions.<br/>It starts with a team that needed something faster than a ticket.</div></Fade>
          </div>
        )}
        ...
        {show("the-discovery") && (
          <div style={{ display: "flex", alignItems: "center", gap: 50, width: "100%", padding: "0 60px" }}>
            <div style={{ flex: 1 }}>
              <Fade show delay={200} duration={600}><div style={{ fontSize: 15, fontWeight: 600, letterSpacing: 3, color: "#f59e0b", textTransform: "uppercase", marginBottom: 16, fontFamily: "'JetBrains Mono'" }}>Discovery Timeline</div></Fade>
              <Fade show delay={500} duration={500}>
                <div style={{ background: "#0f0f24", border: "1px solid #1e1e3a", borderRadius: 14, padding: 20 }}>
                  {[
                    { time: "Day 0", event: "Sales rep shares client proposal via CloudShare", detail: "Free tier — no approval needed", color: "#22c55e", icon: "📤" },
                    { time: "Week 2", event: "5 reps using it daily", detail: "\"Way faster than our approved tool\"", color: "#3b82f6", icon: "👥" },
                    { time: "Month 1", event: "14 users, 400+ files uploaded", detail: "Client contracts, pricing sheets, PII", color: "#f59e0b", icon: "📁" },
                  ].map((e, i) => (
                    <Fade key={i} show delay={600 + i * 600} duration={400} direction="left" distance={12}>
                      <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 0", borderBottom: i < 2 ? "1px solid #1a1a2e" : "none" }}>
                        ...
                        <div style={{ minWidth: 70, fontSize: 14, color: "#64748b", fontFamily: "'JetBrains Mono'" }}>{e.time}</div>
                        <div>
              ...
              <Fade show delay={2600} duration={400}>
                <div style={{ marginTop: 10, display: "flex", gap: 8, flexWrap: "wrap" }}>
                  {["Client contracts", "Pricing sheets", "Customer PII", "Product roadmap"].map((tag, i) => (
                    <div key={i} style={{ padding: "4px 12px", background: "#ef444410", border: "1px solid #ef444425", borderRadius: 6, fontSize: 13, color: "#ef4444", fontFamily: "'JetBrains Mono'" }}>{tag}</div>
                  ))}
                </div>
              </Fade>
            </div>
            <div style={{ flex: 1, paddingLeft: 20 }}>
              <Fade show delay={1200} duration={700}><div style={{ fontSize: 39, fontWeight: 700, color: "#f1f5f9", fontFamily: "'Space Grotesk'", marginBottom: 12, lineHeight: 1.3 }}>14 users. 400 files.<br/><span style={{ color: "#ef4444" }}>Zero IT oversight.</span></div></Fade>
              <Fade show delay={2000} duration={600}><div style={{ fontSize: 21, color: "#94a3b8", lineHeight: 1.7 }}>The tool was never evaluated, approved, or secured. It just spread — one shared link at a time.</div></Fade>
            </div>
          </div>
        )}
        ...
        {show("the-risk") && (
          <div style={{ display: "flex", alignItems: "center", gap: 50, width: "100%", padding: "0 60px" }}>
            <div style={{ flex: 1 }}>
              <Fade show delay={200} duration={600}><div style={{ fontSize: 15, fontWeight: 600, letterSpacing: 3, color: "#ef4444", textTransform: "uppercase", marginBottom: 16, fontFamily: "'JetBrains Mono'" }}>Risk Assessment</div></Fade>
              {[
                { label: "Data classification", value: "120 files contain PII", sublabel: "Customer names, emails, addresses", color: "#ef4444" },
                { label: "Encryption", value: "None", sublabel: "Free tier — no encryption at rest", color: "#ef4444" },
                { label: "Compliance", value: "GDPR Article 32", sublabel: "Encryption required for personal data", color: "#f59e0b" },
                { label: "Access controls", value: "Link-based sharing", sublabel: "Anyone with the link can view", color: "#ef4444" },
                { label: "Offboarding", value: "Not in revocation checklist", sublabel: "Tool unknown to IT", color: "#f59e0b" },
              ].map((row, i) => (
                <Fade key={i} show delay={400 + i * 450} duration={400}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", padding: "8px 0", borderBottom: i < 4 ? "1px solid #1e1e3a" : "none" }}>
                    <div>
                      <span style={{ fontSize: 14, color: "#64748b", fontFamily: "'JetBrains Mono'" }}>{row.label}</span>
                      <div style={{ fontSize: 13, color: "#4a5568" }}>{row.sublabel}</div>
                    </div>
                    <span style={{ fontSize: 15, color: row.color, fontWeight: 600, textAlign: "right" }}>{row.value}</span>
                  </div>
                </Fade>
              ))}
            </div>
            <div style={{ flex: 1, paddingLeft: 20 }}>
              <Fade show delay={1200} duration={700}><div style={{ fontSize: 39, fontWeight: 700, color: "#f1f5f9", fontFamily: "'Space Grotesk'", marginBottom: 12, lineHeight: 1.3 }}>The team moved faster.<br/><span style={{ color: "#ef4444" }}>But the data moved out.</span></div></Fade>
              <Fade show delay={2000} duration={600}><div style={{ fontSize: 21, color: "#94a3b8", lineHeight: 1.7 }}>Customer data is now in a system with no encryption, no access controls, and no audit trail. No data processing agreement in place.</div></Fade>
            </div>
          </div>
        )}
          ...
          <div style={{ textAlign: "center", width: "100%", padding: "0 50px" }}>
            ...
            <Fade show delay={600} duration={800}>
              <div style={{ maxWidth: 680, margin: "0 auto 24px", padding: "20px 28px", background: "#6366f108", border: "1px solid #6366f120", borderRadius: 14, textAlign: "left" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
                  ...
                  <span style={{ fontSize: 20, fontWeight: 700, color: "#ef4444" }}>SHADOW IT: Unauthorized file-sharing tool in use by 14 employees</span>
                </div>
                <div style={{ fontSize: 19, color: "#cbd5e1", lineHeight: 1.8 }}>
                  CloudShare (free tier) — 400+ files uploaded, 120 containing PII. No encryption, no access controls, no DPA in place. GDPR compliance risk: HIGH. Tool not in approved vendor list. Not in offboarding revocation checklist.
                </div>
              </div>
            </Fade>
            <Fade show delay={2200} duration={700}>
              <div style={{ display: "flex", justifyContent: "center", gap: 24, flexWrap: "wrap" }}>
                {[
                  { value: "14", label: "Users", color: "#f59e0b" },
                  { value: "400+", label: "Files", color: "#ef4444" },
                  { value: "120", label: "PII files", color: "#ef4444" },
                  { value: "HIGH", label: "GDPR risk", color: "#ef4444" },
                ].map((stat, i) => (
        ...
        {show("action") && (
          <div style={{ textAlign: "center", width: "100%", padding: "0 50px" }}>
            ...
            <Fade show delay={500} duration={800}><div style={{ fontSize: 45, fontWeight: 700, color: "#f1f5f9", fontFamily: "'Space Grotesk'", marginBottom: 28, lineHeight: 1.3 }}>Contain, migrate,<br/><span style={{ color: "#6366f1" }}>then prevent.</span></div></Fade>
            <Fade show delay={1200} duration={500}>
              <div style={{ maxWidth: 640, margin: "0 auto" }}>
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  {[
                    { n: "1", text: "Migrate all files to approved platform immediately", color: "#ef4444" },
                    { n: "2", text: "Revoke CloudShare access for all 14 users", color: "#ef4444" },
                    { n: "3", text: "Audit uploaded files for PII exposure", color: "#f59e0b" },
                    { n: "4", text: "Fast-track approved alternative that meets the team\u2019s speed needs", color: "#22c55e" },
                    { n: "5", text: "Add shadow IT scanning to quarterly security review", color: "#6366f1" },
                  ].map((a, i) => (
        ...
        {show("close") && (
          <div style={{ textAlign: "center" }}>
            <Fade show delay={200} duration={800}><div style={{ fontSize: 53, fontWeight: 700, color: "#f1f5f9", fontFamily: "'Space Grotesk'", marginBottom: 16, lineHeight: 1.3 }}>IT tracks approved tools.<br/><span style={{ color: "#6366f1" }}>Qorpera tracks what people actually use.</span></div></Fade>
            <Fade show delay={1000} duration={700}><div style={{ fontSize: 24, color: "#94a3b8", maxWidth: 560, margin: "0 auto 28px", lineHeight: 1.7 }}>Shadow IT spreads because teams need tools faster than procurement can deliver. Activity intelligence catches the gap between what&apos;s approved and what&apos;s actually running.</div></Fade>
